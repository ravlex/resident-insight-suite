...
function Index() {
  ...
  const handleAiSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      const query = aiInputValue.toLowerCase();
      if (query.includes("ленина") || query.includes("рост")) {
      ...
      } else if (query.includes("сбор") || query.includes("прогноз")) {
        setAiResponse("Прогноз сборов на июль составляет 4.55 млн ₽. Ожидается сезонное снижение оплат по отоплению, но рост по услугам ГВС из-за периода отпусков.");
      } else if (query.includes("жалоб") || query.includes("проблем")) {
        setAiResponse("Наибольшее количество жалоб (14) зафиксировано по адресу ул. Ленина, д. 15. Большинство касается качества горячей воды.");
      } else {
        setAiResponse(`Я проанализировал ваш запрос "${aiInputValue}". Рекомендую обратить внимание на собираемость в Ворошиловском районе, где зафиксировано снижение на 4.2% по сравнению с прошлым месяцем.`);
      }
    }, 1500);
  };
